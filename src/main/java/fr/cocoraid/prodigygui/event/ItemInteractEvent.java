...
public class ItemInteractEvent implements Listener {
   ...
   public void interact(PlayerInteractEvent e) {
      if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && e.getItem() != null && e.getItem().hasItemMeta()) {
         ThreeDimensionalMenu.getMenus().stream().filter((m) -> {
            return m.getOpenItem() == e.getItem().getType() && m.getOpenItemName().equalsIgnoreCase(e.getItem().getItemMeta().getDisplayName());
         }).findAny().ifPresent((m) -> {
